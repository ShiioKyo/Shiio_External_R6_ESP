...
using Shiio_External_R6_ESP.Imports;
using Shiio_External_R6_ESP.Memory;
using System;
...
namespace Shiio_External_R6_ESP
{
    ...
    public class MemoryHelper
    {
        ...
        public void wpmf(long address, float f)
        {
            //A couple checks to try and avoid crashing
            if (address == 0 || (IntPtr)address == IntPtr.Zero)
            {
                return;
            }
            Kernel32.WriteProcessMemory(GameHandle, (IntPtr)address, BitConverter.GetBytes(f), sizeof(float), out int bytesRead);
        }
        ...
        {
            int s = sizeof(float) * 3 + sizeof(uint) + sizeof(float) * 3 + sizeof(uint) + sizeof(float) * 3 + sizeof(uint) + sizeof(float) * 3;
            byte[] buffer = new byte[s];
        ...
        public void Glow(float red, float green, float blue)
        {
            long glowbase = rpml(GetBaseAddress() + Offsets.ADDRESS_GLOWMANAGER);
            long glowbase1 = rpml(glowbase + Offsets.OFFSET_GLOWMANAGER_GLOWBASE);
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_COLOR_RED, red / 255.0f); //Red
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_COLOR_GREEN, green / 255.0f); //Green
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_COLOR_BLUE, blue / 255.0f); //Blue
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_OPACITY, 0); //Opacity
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_STARTDISTANCE, 5); //StartDistance
            wpmf(glowbase1 + Offsets.OFFSET_GLOWMANAGER_DISTANCE, 0); // Distance
        }
